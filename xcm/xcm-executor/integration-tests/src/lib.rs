...
use diamond_test_client::{
	BlockBuilderExt, ClientBlockImportExt, DefaultTestClientBuilderExt, ExecutionStrategy,
	InitdiamondBlockBuilder, TestClientBuilder, TestClientBuilderExt,
};
use diamond_test_runtime::pallet_test_notifier;
use diamond_test_service::construct_extrinsic;
...
fn basic_buy_fees_message_executes() {
	...
	let mut block_builder = client.init_diamond_block_builder();

	let execute = construct_extrinsic(
		&client,
		diamond_test_runtime::Call::Xcm(pallet_xcm::Call::execute {
	...
	block_builder.push_diamond_extrinsic(execute).expect("pushes extrinsic");
	...
	client
		...
		.inspect_state(|| {
			assert!(diamond_test_runtime::System::events().iter().any(|r| matches!(
				r.event,
				diamond_test_runtime::Event::Xcm(pallet_xcm::Event::Attempted(Outcome::Complete(
					_
				))),
			)));
		});
...
fn query_response_fires() {
	...
	use diamond_test_runtime::Event::TestNotifier;
	...
	let execute = construct_extrinsic(
		&client,
		diamond_test_runtime::Call::TestNotifier(pallet_test_notifier::Call::prepare_new_query {}),
		sp_keyring::Sr25519Keyring::Alice,
		0,
	);
	...
	client
		...
		.expect("state should exist")
		.inspect_state(|| {
			for r in diamond_test_runtime::System::events().iter() {
				match r.event {
	...
	client
		...
		.inspect_state(|| {
			assert!(diamond_test_runtime::System::events().iter().any(|r| matches!(
				r.event,
				diamond_test_runtime::Event::Xcm(pallet_xcm::Event::ResponseReady(
					q,
					Response::ExecutionResult(Ok(())),
				)) if q == query_id,
			)));
			assert_eq!(
				diamond_test_runtime::Xcm::query(query_id),
...
fn query_response_elicits_handler() {
	...
	let execute = construct_extrinsic(
		&client,
		diamond_test_runtime::Call::TestNotifier(
