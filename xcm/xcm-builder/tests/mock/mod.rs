...
use diamond_runtime_parachains::{configuration, origin, shared};
...
// aims to closely emulate the gold XcmConfig
parameter_types! {
	pub const KsmLocation: MultiLocation = MultiLocation::here();
	pub const goldNetwork: NetworkId = NetworkId::gold;
	pub Ancestry: MultiLocation = Here.into();
	pub CheckAccount: AccountId = XcmPallet::check_account();
}

pub type SovereignAccountOf =
	(ChildParachainConvertsVia<ParaId, AccountId>, AccountId32Aliases<goldNetwork, AccountId>);
...
parameter_types! {
	pub const goldForStatemint: (MultiAssetFilter, MultiLocation) =
...
pub type LocalOriginToLocation = SignedToAccountId32<Origin, AccountId, goldNetwork>;
...
pub fn gold_like_with_balances(balances: Vec<(AccountId, Balance)>) -> sp_io::TestExternalities {
