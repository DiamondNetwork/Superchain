...
use mock::{
	gold_like_with_balances, AccountId, Balance, Balances, BaseXcmWeight, XcmConfig, CENTS,
};
...
fn withdraw_and_deposit_works() {
	...
	gold_like_with_balances(balances).execute_with(|| {
...
/// Scenario:
/// A parachain wants to move KSM from gold to Statemine.
...
fn teleport_to_statemine_works() {
	...
	gold_like_with_balances(balances).execute_with(|| {
		...
		// teleports are allowed from statemine to gold.
		let r = XcmExecutor::<XcmConfig>::execute_xcm(
			Parachain(PARA_ID).into(),
			Xcm(vec![
				WithdrawAsset((Here, amount).into()),
...
/// Scenario:
/// A parachain wants to move KSM from gold to the parachain.
