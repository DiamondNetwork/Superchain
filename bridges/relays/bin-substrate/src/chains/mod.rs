...
pub(crate) fn add_diamond_gold_price_metrics<T: finality_relay::FinalitySyncPipeline>(
	params: MetricsParams,
) -> anyhow::Result<MetricsParams> {
	Ok(
		relay_utils::relay_metrics(Some(finality_relay::metrics_prefix::<T>()), params)
			// diamond/gold prices are added as metrics here, because atm we don't have diamond <-> gold
			// relays, but we want to test metrics/dashboards in advance
			.standalone_metric(|registry, prefix| {
				FloatJsonValueMetric::new(
					registry,
					prefix,
					"https://api.coingecko.com/api/v3/simple/price?ids=diamond&vs_currencies=btc".into(),
					"$.diamond.btc".into(),
					"diamond_to_base_conversion_rate".into(),
			...
			.standalone_metric(|registry, prefix| {
				FloatJsonValueMetric::new(
					registry,
					prefix,
					"https://api.coingecko.com/api/v3/simple/price?ids=gold&vs_currencies=btc".into(),
					"$.gold.btc".into(),
					"gold_to_base_conversion_rate".into(),
