...
//! Wococo-to-titan messages sync entrypoint.
...
/// Wococo-to-titan message lane.
pub type WococoMessagesTotitan =
	SubstrateMessageLaneToSubstrate<Wococo, WococoSigningParams, titan, titanSigningParams>;

impl SubstrateMessageLane for WococoMessagesTotitan {
	const OUTBOUND_LANE_MESSAGE_DETAILS_METHOD: &'static str = bp_titan::TO_titan_MESSAGE_DETAILS_METHOD;
	const OUTBOUND_LANE_LATEST_GENERATED_NONCE_METHOD: &'static str =
		bp_titan::TO_titan_LATEST_GENERATED_NONCE_METHOD;
	const OUTBOUND_LANE_LATEST_RECEIVED_NONCE_METHOD: &'static str = bp_titan::TO_titan_LATEST_RECEIVED_NONCE_METHOD;
	...
	const BEST_FINALIZED_TARGET_HEADER_ID_AT_SOURCE: &'static str = bp_titan::BEST_FINALIZED_titan_HEADER_METHOD;

	type SourceChain = Wococo;
	type TargetChain = titan;
	...
	fn make_messages_receiving_proof_transaction(
		...
		_generated_at_block: titanHeaderId,
		proof: <Self as MessageLane>::MessagesReceivingProof,
	) -> Bytes {
		let (relayers_state, proof) = proof;
		let call = relay_wococo_client::runtime::Call::BridgeMessagestitan(
			relay_wococo_client::runtime::BridgeMessagestitanCall::receive_messages_delivery_proof(
				proof,
				relayers_state,
			),
		);
		...
		log::trace!(
			target: "bridge",
			"Prepared titan -> Wococo confirmation transaction. Weight: <unknown>/{}, size: {}/{}",
	...
	fn target_transactions_author(&self) -> bp_titan::AccountId {
	...
	fn make_messages_delivery_transaction(
		...
		let call = relay_titan_client::runtime::Call::BridgeMessagesWococo(
			relay_titan_client::runtime::BridgeMessagesWococoCall::receive_messages_proof(
		...
		let transaction = titan::sign_transaction(genesis_hash, &self.target_sign, transaction_nonce, call);
		log::trace!(
			target: "bridge",
			"Prepared Wococo -> titan delivery transaction. Weight: <unknown>/{}, size: {}/{}",
...
type WococoSourceClient =
	SubstrateMessagesSource<Wococo, WococoMessagesTotitan, relay_wococo_client::runtime::WithtitanMessagesInstance>;

/// titan node as messages target.
type titanTargetClient =
	SubstrateMessagesTarget<titan, WococoMessagesTotitan, relay_titan_client::runtime::WithWococoMessagesInstance>;

/// Run Wococo-to-titan messages sync.
pub async fn run(
	params: MessagesRelayParams<Wococo, WococoSigningParams, titan, titanSigningParams>,
) -> Result<(), String> {
	...
	let lane = WococoMessagesTotitan {
	...
	let max_messages_size_in_single_batch = bp_titan::max_extrinsic_size() / 3;
	// we don't know exact weights of the titan runtime. So to guess weights we'll be using
	...
	log::info!(
		target: "bridge",
		"Starting Wococo -> titan messages relay.\n\t\
	...
	messages_relay::message_lane_loop::run(
		messages_relay::message_lane_loop::Params {
			...
			target_tick: titan::AVERAGE_BLOCK_INTERVAL,
			reconnect_delay: relay_utils::relay_loop::RECONNECT_DELAY,
			stall_timeout,
			delivery_params: messages_relay::message_lane_loop::MessageDeliveryParams {
				max_unrewarded_relayer_entries_at_target: bp_titan::MAX_UNREWARDED_RELAYER_ENTRIES_AT_INBOUND_LANE,
				max_unconfirmed_nonces_at_target: bp_titan::MAX_UNCONFIRMED_MESSAGES_AT_INBOUND_LANE,
				max_messages_in_single_batch,
				max_messages_weight_in_single_batch,
				max_messages_size_in_single_batch,
