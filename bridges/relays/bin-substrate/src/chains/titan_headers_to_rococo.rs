...
//! Wococo-to-titan headers sync entrypoint.

use crate::finality_pipeline::{SubstrateFinalitySyncPipeline, SubstrateFinalityToSubstrate};

use bp_header_chain::justification::GrandpaJustification;
use codec::Encode;
use relay_titan_client::{titan, SigningParams as titanSigningParams};
...
pub(crate) const MAXIMAL_BALANCE_DECREASE_PER_DAY: bp_titan::Balance = 1_500_000_000_000_000;

/// Wococo-to-titan finality sync pipeline.
pub(crate) type WococoFinalityTotitan = SubstrateFinalityToSubstrate<Wococo, titan, titanSigningParams>;

impl SubstrateFinalitySyncPipeline for WococoFinalityTotitan {
	...
	fn transactions_author(&self) -> bp_titan::AccountId {
	...
	fn make_submit_finality_proof_transaction(
		...
		let call = relay_titan_client::runtime::Call::BridgeGrandpaWococo(
			relay_titan_client::runtime::BridgeGrandpaWococoCall::submit_finality_proof(header.into_inner(), proof),
		);
...
mod tests {
	...
	fn maximal_balance_decrease_per_day_is_sane() {
		// titan/Wococo GRANDPA pallet weights. They're now using Rialto weights => using `RialtoWeight` is justified.
		//
		// Using Rialto runtime this is slightly incorrect, because `DbWeight` of titan/Wococo runtime may differ
		// from the `DbWeight` of Rialto runtime. But now (and most probably forever) it is the same.
		type titanGrandpaPalletWeights = pallet_bridge_grandpa::weights::RialtoWeight<rialto_runtime::Runtime>;
		...
		let single_source_header_submit_call_weight =
			titanGrandpaPalletWeights::submit_finality_proof(AVG_VOTES_ANCESTRIES_LEN, AVG_PRECOMMITS_LEN);
		...
		let single_source_header_tx_cost = bp_titan::WeightToFee::calc(&single_source_header_submit_tx_weight);
