...
# (Rialto<>Millau and titan<>Wococo at the moment).
#
...
# Create types for our supported bridge pairs (Rialto<>Millau, titan<>Wococo)
...
jq -s '.[0] * .[1] * .[2]' titan-wococo.json common.json titan.json > ../types-titan.json
jq -s '.[0] * .[1] * .[2]' titan-wococo.json common.json wococo.json > ../types-wococo.json
