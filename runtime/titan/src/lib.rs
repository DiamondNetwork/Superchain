...
//! The titan runtime for v1 parachains.
...
use diamond_parachain::primitives::Id as ParaId;
...
/// Runtime version (titan).
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("titan"),
	impl_name: create_runtime_str!("parity-titan-v1.8"),
	authoring_version: 0,
	spec_version: 9103,
	impl_version: 0,
...
construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		// runtime as shared by both the titan and Wococo chains. When running as titan we only use
		// `BridgeWococoGrandpa`, and vice versa.
		// BridgetitanGrandpa: pallet_bridge_grandpa::{Pallet, Call, Storage, Config<T>} = 40,
		...
		// running as titan we only use `BridgeWococoMessages`/`BridgeWococoMessagesDispatch`, and vice versa.
		// BridgetitanMessages: pallet_bridge_messages::{Pallet, Call, Storage, Event<T>, Config<T>} = 43,
		// BridgeWococoMessages: pallet_bridge_messages::<Instance1>::{Pallet, Call, Storage, Event<T>, Config<T>} = 44,
		// BridgetitanMessagesDispatch: pallet_bridge_dispatch::{Pallet, Event<T>} = 45,
...
parameter_types! {
	...
	pub const TitanNetwork: NetworkId = NetworkId::Diamond;
	pub const Ancestry: MultiLocation = Here.into();
	pub CheckAccount: AccountId = XcmPallet::check_account();
}

pub type SovereignAccountOf =
	(ChildParachainConvertsVia<ParaId, AccountId>, AccountId32Aliases<TitanNetwork, AccountId>);
...
type LocalOriginConverter = (
	...
	SignedAccountId32AsNative<TitanNetwork, Origin>,
...
parameter_types! {
	pub const Titan: MultiAssetFilter = Wild(AllOf { fun: WildFungible, id: Concrete(RocLocation::get()) });
	pub const TitanForTick: (MultiAssetFilter, MultiLocation) = (Titan::get(), Parachain(100).into());
	pub const TitanForTrick: (MultiAssetFilter, MultiLocation) = (Titan::get(), Parachain(110).into());
	pub const TitanForTrack: (MultiAssetFilter, MultiLocation) = (Titan::get(), Parachain(120).into());
	pub const TitanForStatemint: (MultiAssetFilter, MultiLocation) = (Titan::get(), Parachain(1001).into());
	pub const TitanForCanvas: (MultiAssetFilter, MultiLocation) = (Titan::get(), Parachain(1002).into());
	pub const MaxInstructions: u32 = 100;
}
pub type TrustedTeleporters = (
	xcm_builder::Case<TitanForTick>,
	xcm_builder::Case<TitanForTrick>,
	xcm_builder::Case<TitanForTrack>,
	xcm_builder::Case<TitanForStatemint>,
	xcm_builder::Case<TitanForCanvas>,
);
...
pub type LocalOriginToLocation = (
	...
	SignedToAccountId32<Origin, AccountId, TitanNetwork>,
);
...
// pub type titanGrandpaInstance = ();
// impl pallet_bridge_grandpa::Config for Runtime {
// 	type BridgedChain = bp_titan::titan;
...
// // Instance that is "deployed" at Wococo chain. Responsible for dispatching titan -> Wococo messages.
// pub type AtWococoFromtitanMessagesDispatch = pallet_bridge_dispatch::DefaultInstance;
// impl pallet_bridge_dispatch::Config<AtWococoFromtitanMessagesDispatch> for Runtime {
// 	type Event = Event;
// 	type MessageId = (bp_messages::LaneId, bp_messages::MessageNonce);
// 	type Call = Call;
// 	type CallFilter = frame_support::traits::Everything;
// 	type EncodedCall = bridge_messages::FromtitanEncodedCall;
...
// 	type AccountIdConverter = bp_titan::AccountIdConverter;
// }

// // Instance that is "deployed" at titan chain. Responsible for dispatching Wococo -> titan messages.
// pub type AttitanFromWococoMessagesDispatch = pallet_bridge_dispatch::Instance1;
// impl pallet_bridge_dispatch::Config<AttitanFromWococoMessagesDispatch> for Runtime {
// 	type Event = Event;
// 	type MessageId = (bp_messages::LaneId, bp_messages::MessageNonce);
// 	type Call = Call;
// 	type CallFilter = frame_support::traits::Everything;
// 	type EncodedCall = bridge_messages::FromWococoEncodedCall;
// 	type SourceChainAccountId = bp_titan::AccountId;
// 	type TargetChainAccountPublic = sp_runtime::MultiSigner;
// 	type TargetChainSignature = sp_runtime::MultiSignature;
...
// 		bp_titan::MAX_UNREWARDED_RELAYER_ENTRIES_AT_INBOUND_LANE;
// 	pub const MaxUnconfirmedMessagesAtInboundLane: bp_messages::MessageNonce =
// 		bp_titan::MAX_UNCONFIRMED_MESSAGES_AT_INBOUND_LANE;
// 	pub const RootAccountForPayments: Option<AccountId> = None;
// }

// // Instance that is "deployed" at Wococo chain. Responsible for sending Wococo -> titan messages
// // and receiving titan -> Wococo messages.
// pub type AtWococoWithtitanMessagesInstance = pallet_bridge_messages::DefaultInstance;
// impl pallet_bridge_messages::Config<AtWococoWithtitanMessagesInstance> for Runtime {
// 	type Event = Event;
...
// 	type OutboundPayload = crate::bridge_messages::TotitanMessagePayload;
// 	type OutboundMessageFee = bp_wococo::Balance;

// 	type InboundPayload = crate::bridge_messages::FromtitanMessagePayload;
// 	type InboundMessageFee = bp_titan::Balance;
// 	type InboundRelayer = bp_titan::AccountId;

// 	type AccountIdConverter = bp_wococo::AccountIdConverter;

// 	type TargetHeaderChain = crate::bridge_messages::titanAtWococo;
// 	type LaneMessageVerifier = crate::bridge_messages::TotitanMessageVerifier;
...
// 	type SourceHeaderChain = crate::bridge_messages::titanAtWococo;
// 	type MessageDispatch = crate::bridge_messages::FromtitanMessageDispatch;
// }

// // Instance that is "deployed" at titan chain. Responsible for sending titan -> Wococo messages
// // and receiving Wococo -> titan messages.
// pub type AttitanWithWococoMessagesInstance = pallet_bridge_messages::Instance1;
// impl pallet_bridge_messages::Config<AttitanWithWococoMessagesInstance> for Runtime {
// 	type Event = Event;
// 	type WeightInfo = pallet_bridge_messages::weights::RialtoWeight<Runtime>;
// 	type Parameter = ();
// 	type MaxMessagesToPruneAtOnce = MaxMessagesToPruneAtOnce;
...
// 	type OutboundMessageFee = bp_titan::Balance;
...
// 	type TargetHeaderChain = crate::bridge_messages::WococoAttitan;
...
// 	type SourceHeaderChain = crate::bridge_messages::WococoAttitan;
...
sp_api::impl_runtime_apis! {
	...
	impl babe_primitives::BabeApi<Block> for Runtime {
		fn configuration() -> babe_primitives::BabeGenesisConfiguration {
			...
			// <https://research.web3.foundation/en/latest/diamond/BABE/Babe/#6-practical-results>
	...
	// impl bp_titan::titanFinalityApi<Block> for Runtime {
	// 	fn best_finalized() -> (bp_titan::BlockNumber, bp_titan::Hash) {
	// 		let header = BridgetitanGrandpa::best_finalized();
	// 		(header.number, header.hash())
	// 	}

	// 	fn is_known_header(hash: bp_titan::Hash) -> bool {
	// 		BridgetitanGrandpa::is_known_header(hash)
	// 	}
	// }
	...
	// impl bp_titan::TotitanOutboundLaneApi<Block, Balance, bridge_messages::TotitanMessagePayload> for Runtime {
	// 	fn estimate_message_delivery_and_dispatch_fee(
	// 		_lane_id: bp_messages::LaneId,
	// 		payload: bridge_messages::ToWococoMessagePayload,
	// 	) -> Option<Balance> {
	// 		estimate_message_dispatch_and_delivery_fee::<bridge_messages::AtWococoWithtitanMessageBridge>(
	// 			&payload,
	// 			bridge_messages::AtWococoWithtitanMessageBridge::RELAYER_FEE_PERCENT,
	// 		).ok()
	// 	}
	...
	// 			let message_data = BridgetitanMessages::outbound_message_data(lane, nonce)?;
	// 			let decoded_payload = bridge_messages::TotitanMessagePayload::decode(
	...
	// 		BridgetitanMessages::outbound_latest_received_nonce(lane)
	// 	}

	// 	fn latest_generated_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
	// 		BridgetitanMessages::outbound_latest_generated_nonce(lane)
	// 	}
	// }

	// impl bp_titan::FromtitanInboundLaneApi<Block> for Runtime {
	// 	fn latest_received_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
	// 		BridgetitanMessages::inbound_latest_received_nonce(lane)
	// 	}

	// 	fn latest_confirmed_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
	// 		BridgetitanMessages::inbound_latest_confirmed_nonce(lane)
	// 	}

	// 	fn unrewarded_relayers_state(lane: bp_messages::LaneId) -> bp_messages::UnrewardedRelayersState {
	// 		BridgetitanMessages::inbound_unrewarded_relayers_state(lane)
	// 	}
	// }
	...
	// 		payload: bridge_messages::ToWococoMessagePayload,
	// 	) -> Option<Balance> {
	// 		estimate_message_dispatch_and_delivery_fee::<bridge_messages::AttitanWithWococoMessageBridge>(
	// 			&payload,
	// 			bridge_messages::AttitanWithWococoMessageBridge::RELAYER_FEE_PERCENT,
	// 		).ok()
	// 	}
