...
//! Over-bridge messaging support for titan <> Wococo bridge.

pub use self::{at_titan::*, at_wococo::*};
...
use bp_runtime::{ChainId, titan_CHAIN_ID, WOCOCO_CHAIN_ID};
...
/// Maximal weight of single message delivery confirmation transaction on titan/Wococo chain.
///
...
/// Increase of delivery transaction weight on titan/Wococo chain with every additional message byte.
///
...
/// Weight of single regular message delivery transaction on titan/Wococo chain.
///
...
/// Weight of pay-dispatch-fee operation for inbound messages at titan/Wococo chain.
///
...
/// Number of bytes, included in the signed titan/Wococo transaction apart from the encoded call itself.
///
/// Can be computed by subtracting encoded call size from raw transaction size.
const TX_EXTRA_BYTES: u32 = 130;

/// titan chain as it is seen at titan.
pub type titanAttitan =
	titanLikeChain<AttitanWithWococoMessageBridge, crate::titanGrandpaInstance>;

/// titan chain as it is seen at Wococo.
pub type titanAtWococo =
	titanLikeChain<AtWococoWithtitanMessageBridge, crate::titanGrandpaInstance>;

/// Wococo chain as it is seen at Wococo.
pub type WococoAtWococo =
	titanLikeChain<AtWococoWithtitanMessageBridge, crate::WococoGrandpaInstance>;

/// Wococo chain as it is seen at titan.
pub type WococoAttitan =
	titanLikeChain<AttitanWithWococoMessageBridge, crate::WococoGrandpaInstance>;

/// titan/Wococo chain from message lane point of view.
#[derive(RuntimeDebug, Clone, Copy)]
pub struct titanLikeChain<B, GI> {
	_bridge_definition: PhantomData<B>,
	_at_this_chain_grandpa_pallet_instance: PhantomData<GI>,
}

impl<B, GI> ChainWithMessages for titanLikeChain<B, GI> {
...
impl<B, GI> ThisChainWithMessages for titanLikeChain<B, GI> {
...
impl<B, GI> BridgedChainWithMessages for titanLikeChain<B, GI> {
	...
	fn message_weight_limits(_message_payload: &[u8]) -> RangeInclusive<Weight> {
		...
		// we're charging for payload bytes in `With(Wococo | titan)MessageBridge::transaction_payment` function
		//
...
impl<B, GI> SourceHeaderChain<crate::Balance> for titanLikeChain<B, GI>
where
	B: MessageBridge,
...
impl Get<crate::Balance> for GetDeliveryConfirmationTransactionFee {
	fn get() -> crate::Balance {
		<titanAttitan as ThisChainWithMessages>::transaction_payment(
			titanAttitan::estimate_delivery_confirmation_transaction(),
		)
	}
}

/// This module contains definitions that are used by the messages pallet instance, "deployed" at titan.
mod at_titan {
	use super::*;

	/// Message bridge that is "deployed" at titan chain and connecting it to Wococo chain.
	#[derive(RuntimeDebug, Clone, Copy)]
	pub struct AttitanWithWococoMessageBridge;

	impl MessageBridge for AttitanWithWococoMessageBridge {
		const THIS_CHAIN_ID: ChainId = titan_CHAIN_ID;
		const BRIDGED_CHAIN_ID: ChainId = WOCOCO_CHAIN_ID;
		const RELAYER_FEE_PERCENT: u32 = 10;

		type ThisChain = titanAttitan;
		type BridgedChain = WococoAttitan;
		type BridgedMessagesInstance = crate::AtWococoWithtitanMessagesInstance;
	...
	/// Message payload for titan -> Wococo messages as it is seen at the titan.
	pub type ToWococoMessagePayload =
		messages_source::FromThisChainMessagePayload<AttitanWithWococoMessageBridge>;

	/// Message verifier for titan -> Wococo messages at titan.
	pub type ToWococoMessageVerifier =
		messages_source::FromThisChainMessageVerifier<AttitanWithWococoMessageBridge>;

	/// Message payload for Wococo -> titan messages as it is seen at titan.
	pub type FromWococoMessagePayload =
		messages_target::FromBridgedChainMessagePayload<AttitanWithWococoMessageBridge>;

	/// Encoded titan Call as it comes from Wococo.
	pub type FromWococoEncodedCall =
		messages_target::FromBridgedChainEncodedMessageCall<crate::Call>;

	/// Call-dispatch based message dispatch for Wococo -> titan messages.
	pub type FromWococoMessageDispatch = messages_target::FromBridgedChainMessageDispatch<
		AttitanWithWococoMessageBridge,
		crate::Runtime,
		pallet_balances::Pallet<crate::Runtime>,
		crate::AttitanFromWococoMessagesDispatch,
	>;
}
...
mod at_wococo {
	...
	/// Message bridge that is "deployed" at Wococo chain and connecting it to titan chain.
	#[derive(RuntimeDebug, Clone, Copy)]
	pub struct AtWococoWithtitanMessageBridge;

	impl MessageBridge for AtWococoWithtitanMessageBridge {
		const THIS_CHAIN_ID: ChainId = WOCOCO_CHAIN_ID;
		const BRIDGED_CHAIN_ID: ChainId = titan_CHAIN_ID;
		const RELAYER_FEE_PERCENT: u32 = 10;

		type ThisChain = WococoAtWococo;
		type BridgedChain = titanAtWococo;
		type BridgedMessagesInstance = crate::AttitanWithWococoMessagesInstance;

		fn bridged_balance_to_this_balance(
			bridged_balance: bp_titan::Balance,
		) -> bp_wococo::Balance {
			bridged_balance
		}
	}

	/// Message payload for Wococo -> titan messages as it is seen at the Wococo.
	pub type TotitanMessagePayload =
		messages_source::FromThisChainMessagePayload<AtWococoWithtitanMessageBridge>;

	/// Message verifier for Wococo -> titan messages at Wococo.
	pub type TotitanMessageVerifier =
		messages_source::FromThisChainMessageVerifier<AtWococoWithtitanMessageBridge>;

	/// Message payload for titan -> Wococo messages as it is seen at Wococo.
	pub type FromtitanMessagePayload =
		messages_target::FromBridgedChainMessagePayload<AtWococoWithtitanMessageBridge>;

	/// Encoded Wococo Call as it comes from titan.
	pub type FromtitanEncodedCall =
		messages_target::FromBridgedChainEncodedMessageCall<crate::Call>;

	/// Call-dispatch based message dispatch for titan -> Wococo messages.
	pub type FromtitanMessageDispatch = messages_target::FromBridgedChainMessageDispatch<
		AtWococoWithtitanMessageBridge,
		crate::Runtime,
		pallet_balances::Pallet<crate::Runtime>,
		crate::AtWococoFromtitanMessagesDispatch,
	>;
}
...
mod tests {
	...
	#[test]
	fn ensure_titan_messages_weights_are_correct() {
		// **NOTE**: the main purpose of this test is to be sure that any message that is sumbitted
		// to (any) inbound lane in titan<>Wococo bridge can be delivered to the bridged chain.
		...
		// we don't have any knowledge of messages-at-titan weights, so we'll be using
		...
		let max_incoming_message_proof_size = bp_titan::EXTRA_STORAGE_PROOF_SIZE.saturating_add(
			messages::target::maximal_incoming_message_size(bp_titan::max_extrinsic_size()),
		);
		...
		let max_incoming_inbound_lane_data_proof_size =
			bp_messages::InboundLaneData::<()>::encoded_size_hint(
				bp_titan::MAXIMAL_ENCODED_ACCOUNT_ID_SIZE,
				bp_titan::MAX_UNREWARDED_RELAYER_ENTRIES_AT_INBOUND_LANE as _,
				bp_titan::MAX_UNCONFIRMED_MESSAGES_AT_INBOUND_LANE as _,
			)
			.unwrap_or(u32::MAX);
	...
	#[test]
	fn ensure_titan_tx_extra_bytes_constant_is_correct() {
		// **NOTE**: this test checks that we're computing transaction fee (for bridged chain, which, in
		// case of titan<>Wococo, means any chain) on-chain properly. If this assert fails:
		//
		...
		assert!(
			...
			"Hardcoded number of extra bytes in titan transaction {} is lower than actual value: {}",
			TX_EXTRA_BYTES,
