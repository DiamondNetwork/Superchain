...
pub use diamond_node_metrics::{metrics, Metronome};
...
pub use diamond_node_network_protocol::MIN_GOSSIP_PEERS;

pub use determine_new_blocks::determine_new_blocks;

/// These reexports are required so that external crates can use the `delegated_subsystem` macro properly.
pub mod reexports {
	pub use diamond_overseer::gen::{SpawnNamed, SpawnedSubsystem, Subsystem, SubsystemContext};
}
...
impl<Job: JobTrait, Spawner> JobSubsystem<Job, Spawner> {
	...
	where
		...
		<Job as JobTrait>::ToJob:
			Sync + From<<Context as diamond_overseer::SubsystemContext>::Message>,
