...
use diamond_erasure_coding::{branches, obtain_chunks_v1 as obtain_chunks};
use diamond_node_primitives::{BlockData, PoV, Proof};
use diamond_node_subsystem_util::TimeoutExt;
use diamond_primitives::v1::{AuthorityDiscoveryId, HeadData, PersistedValidationData};
use diamond_subsystem::{
	jaeger,
	messages::{AllMessages, RuntimeApiMessage, RuntimeApiRequest},
	ActivatedLeaf, LeafStatus,
};
use diamond_subsystem_testhelpers as test_helpers;
...
fn test_harness_fast_path<T: Future<Output = (VirtualOverseer, RequestResponseConfig)>>(
	...
	let _ = env_logger::builder()
		.is_test(true)
		.filter(Some("diamond_availability_recovery"), log::LevelFilter::Trace)
		.try_init();
