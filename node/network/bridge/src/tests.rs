...
use diamond_node_network_protocol::{request_response::outgoing::Requests, view, ObservedRole};
use diamond_node_subsystem_test_helpers::{
	SingleItemSink, SingleItemStream, TestSubsystemContextHandle,
};
use diamond_node_subsystem_util::metered;
use diamond_primitives::v1::AuthorityDiscoveryId;
...
fn new_test_network() -> (TestNetwork, TestNetworkHandle, TestAuthorityDiscovery) {
	let (net_tx, net_rx) = diamond_node_subsystem_test_helpers::single_item_sink();
...
fn test_harness<T: Future<Output = VirtualOverseer>>(
	...
	let (context, virtual_overseer) =
		diamond_node_subsystem_test_helpers::make_subsystem_context(pool);
