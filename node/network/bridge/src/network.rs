...
use diamond_primitives::v1::{AuthorityDiscoveryId, Block, Hash};
...
pub trait Network: Clone + Send + 'static {
	...
	/// to the diamond protocol - the user of this function should filter only for events related
...
impl Network for Arc<NetworkService<Block, Hash>> {
	fn event_stream(&mut self) -> BoxStream<'static, NetworkEvent> {
		NetworkService::event_stream(self, "diamond-network-bridge").boxed()
	}
