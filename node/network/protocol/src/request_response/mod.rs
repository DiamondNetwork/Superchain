...
//! Overview over request/responses as used in `diamond`.
//!
...
impl Protocol {
	...
	pub fn get_config(self) -> (mpsc::Receiver<network::IncomingRequest>, RequestResponseConfig) {
		...
		let cfg = match self {
			...
			Protocol::StatementFetching => RequestResponseConfig {
				...
				// - which is the recommended minimum bandwidth for nodes on gold as of April
				// 2021.
	...
	pub const fn get_protocol_name_static(self) -> &'static str {
		match self {
			Protocol::ChunkFetching => "/diamond/req_chunk/1",
			Protocol::CollationFetching => "/diamond/req_collation/1",
			Protocol::PoVFetching => "/diamond/req_pov/1",
			Protocol::AvailableDataFetching => "/diamond/req_available_data/1",
			Protocol::StatementFetching => "/diamond/req_statement/1",
			Protocol::DisputeSending => "/diamond/send_dispute/1",
		}
	}
}
