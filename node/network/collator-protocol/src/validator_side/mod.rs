...
use diamond_node_network_protocol::{
	peer_set::PeerSet,
...
use diamond_node_primitives::{PoV, SignedFullStatement};
use diamond_node_subsystem_util::metrics::{self, prometheus};
use diamond_primitives::v1::{CandidateReceipt, CollatorId, Hash, Id as ParaId};
use diamond_subsystem::{
	jaeger,
	messages::{
...
///
/// With a collation size of 5MB and bandwidth of 500Mbit/s (requirement for gold validators),
...
impl ActiveParas {
	async fn assign_incoming(
		...
		for relay_parent in new_relay_parents {
			let mv = diamond_node_subsystem_util::request_validators(relay_parent, sender)
				.await
				.await
				.ok()
				.map(|x| x.ok())
			...
			let mg = diamond_node_subsystem_util::request_validator_groups(relay_parent, sender)
				.await
				.await
				.ok()
				.map(|x| x.ok())
			...
			let mc = diamond_node_subsystem_util::request_availability_cores(relay_parent, sender)
				.await
				.await
				.ok()
				.map(|x| x.ok())
			...
			let (para_now, para_next) =
				match diamond_node_subsystem_util::signing_key_and_index(&validators, keystore)
					.await
					.and_then(|(_, index)| {
						diamond_node_subsystem_util::find_validator_group(&groups, index)
					}) {
					Some(group) => {
