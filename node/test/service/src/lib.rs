...
use diamond_node_primitives::{CollationGenerationConfig, CollatorFn};
use diamond_node_subsystem::messages::{CollationGenerationMessage, CollatorProtocolMessage};
use diamond_overseer::Handle;
use diamond_primitives::v1::{Balance, CollatorPair, HeadData, Id as ParaId, ValidationCode};
use diamond_runtime_common::BlockHashCount;
use diamond_runtime_parachains::paras::ParaGenesisArgs;
use diamond_service::{ClientHandle, Error, ExecuteWithClient, FullClient, IsCollator, NewFull};
use diamond_test_runtime::{
...
/// Declare an instance of the native executor named `diamondTestExecutorDispatch`. Include the wasm binary as the
/// equivalent wasm code.
pub struct diamondTestExecutorDispatch;

impl sc_executor::NativeExecutionDispatch for diamondTestExecutorDispatch {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		diamond_test_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		diamond_test_runtime::native_version()
	}
}

/// The client type being used by the test service.
pub type Client = FullClient<diamond_test_runtime::RuntimeApi, diamondTestExecutorDispatch>;

pub use diamond_service::FullBackend;
...
pub fn new_full(
	...
	diamond_service::new_full::<diamond_test_runtime::RuntimeApi, diamondTestExecutorDispatch, _>(
		config,
		is_collator,
		None,
		true,
		...
		None,
		worker_program_path,
		diamond_service::RealOverseerGen,
	)
}
...
impl ClientHandle for TestClient {
	fn execute_with<T: ExecuteWithClient>(&self, t: T) -> T::Output {
		T::execute_with_client::<_, _, diamond_service::FullBackend>(t, self.0.clone())
	}
}

/// Create a diamond `Configuration`.
///
...
pub fn node_config(
	...
	let mut spec = diamond_local_testnet_config();
	...
	Configuration {
		impl_name: "diamond-test-node".to_string(),
...
pub fn run_validator_node(
	...
	let NewFull { task_manager, client, network, rpc_handlers, overseer_handle, .. } =
		new_full(config, IsCollator::No, worker_program_path)
			.expect("could not create diamond test service");

	let overseer_handle = overseer_handle.expect("test node must have an overseer handle");
	let peer_id = network.local_peer_id().clone();
	let addr = MultiaddrWithPeerId { multiaddr, peer_id };

	diamondTestNode { task_manager, client, overseer_handle, addr, rpc_handlers }
}
...
/// [`diamondTestNode::register_collator`].
...
/// A diamond test node instance used for testing.
pub struct diamondTestNode {
...
impl diamondTestNode {
	/// Send an extrinsic to this node.
	pub async fn send_extrinsic(
		&self,
		function: impl Into<diamond_test_runtime::Call>,
...
pub fn construct_extrinsic(
	...
	UncheckedExtrinsic::new_signed(
		function.clone(),
		diamond_test_runtime::Address::Id(caller.public().into()),
		diamond_primitives::v0::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
...
pub fn construct_transfer_extrinsic(
	...
	let function = diamond_test_runtime::Call::Balances(pallet_balances::Call::transfer {
