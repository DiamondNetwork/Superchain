...
use diamond_primitives::v1::{Block, InherentData as ParachainsInherentData};
use diamond_test_runtime::{GetLastTimestamp, UncheckedExtrinsic};
...
/// An extension for the test client to initialize a diamond specific block builder.
pub trait InitdiamondBlockBuilder {
	/// Init a diamond specific block builder that works for the test runtime.
	///
	/// This will automatically create and push the inherents for you to make the block valid for the test runtime.
	fn init_diamond_block_builder(
		&self,
	) -> sc_block_builder::BlockBuilder<Block, Client, FullBackend>;

	/// Init a diamond specific block builder at a specific block that works for the test runtime.
	///
	/// Same as [`InitdiamondBlockBuilder::init_diamond_block_builder`] besides that it takes a [`BlockId`] to say
	/// which should be the parent block of the block that is being build.
	fn init_diamond_block_builder_at(
		&self,
		at: &BlockId<Block>,
	) -> sc_block_builder::BlockBuilder<Block, Client, FullBackend>;
}

impl InitdiamondBlockBuilder for Client {
	fn init_diamond_block_builder(&self) -> BlockBuilder<Block, Client, FullBackend> {
		let chain_info = self.chain_info();
		self.init_diamond_block_builder_at(&BlockId::Hash(chain_info.best_hash))
	}
	...
	fn init_diamond_block_builder_at(
		...
		let minimum_period = BasicExternalities::new_empty()
			.execute_with(|| diamond_test_runtime::MinimumPeriod::get());
		...
		let slot_duration = BasicExternalities::new_empty()
			.execute_with(|| diamond_test_runtime::SlotDuration::get());
		...
		inherent_data
			.put_data(
				diamond_primitives::v1::PARACHAINS_INHERENT_IDENTIFIER,
...
/// diamond specific extensions for the [`BlockBuilder`].
pub trait BlockBuilderExt {
	/// Push a diamond test runtime specific extrinsic to the block.
	///
