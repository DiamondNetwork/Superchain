...
//! A diamond test client.
//!
//! This test client is using the diamond test runtime.

mod block_builder;

use diamond_primitives::v1::Block;
use sc_service::client;
use sp_core::storage::Storage;
use sp_runtime::BuildStorage;

pub use block_builder::*;
pub use diamond_test_runtime as runtime;
pub use diamond_test_service::{
	construct_extrinsic, construct_transfer_extrinsic, Client, FullBackend,
	diamondTestExecutorDispatch,
};
...
pub type Executor = client::LocalCallExecutor<
	Block,
	FullBackend,
	sc_executor::NativeElseWasmExecutor<diamondTestExecutorDispatch>,
>;

/// Test client builder for diamond.
...
impl substrate_test_client::GenesisInit for GenesisParameters {
	fn genesis_storage(&self) -> Storage {
		diamond_test_service::chain_spec::diamond_local_testnet_genesis()
			.build_storage()
...
mod tests {
	...
	fn ensure_test_client_can_build_and_import_block() {
		...
		let block_builder = client.init_diamond_block_builder();
	...
	fn ensure_test_client_can_push_extrinsic() {
		...
		block_builder.push_diamond_extrinsic(transfer).expect("Pushes extrinsic");
