...
use diamond_runtime_common::claims;
...
type Block = diamond_primitives::v1::Block;
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		diamond_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		diamond_runtime::native_version()
	}
}

/// `ChainInfo` implementation.
pub struct diamondChainInfo;

impl ChainInfo for diamondChainInfo {
	type Block = Block;
	...
	type BlockImport = BlockImport<
		...
		Self::SelectChain,
	>;
	type SignedExtras = diamond_runtime::SignedExtra;
...
where
	T: ChainInfo<
		...
		BlockImport = BlockImport<
			...
			SelectChain,
		>,
		SignedExtras = diamond_runtime::SignedExtra,
	>,
{
...
where
	F: FnOnce(Node<diamondChainInfo>) -> Fut,
	Fut: Future<Output = Result<(), Box<dyn Error>>>,
{
	...
	// parse cli args
	let cmd = <diamond_cli::Cli as StructOpt>::from_args();
	// set up logging
	...
	sc_cli::print_node_infos::<diamond_cli::Cli>(&config);
	let (rpc, task_manager, client, pool, command_sink, backend) =
		client_parts::<diamondChainInfo>(ConfigOrChainSpec::Config(config))?;
	let node =
		Node::<diamondChainInfo>::new(rpc, task_manager, client, pool, command_sink, backend);
...
mod tests {
	use super::*;
	use diamond_service::chain_spec::diamond_development_config;
	...
	fn test_runner() {
		...
		let (rpc, task_manager, client, pool, command_sink, backend) =
			client_parts::<diamondChainInfo>(ConfigOrChainSpec::ChainSpec(
				Box::new(diamond_development_config().unwrap()),
