...
//! Attempts to upgrade the diamond runtime, in a Simnet environment
use std::{error::Error, str::FromStr};

use diamond_runtime::Event;
use diamond_simnet::{dispatch_with_root, run};
...
fn main() -> Result<(), Box<dyn Error>> {
	run(|node| async {
		...
		let wasm_binary = diamond_runtime::WASM_BINARY
			.ok_or("diamond development wasm not available")?
			.to_vec();
		// upgrade runtime.
