...
use diamond_node_jaeger as jaeger;
use diamond_node_primitives::approval::{
	self as approval_types, BlockApprovalMeta, RelayVRFStory,
};
use diamond_node_subsystem::{
	messages::{
...
async fn imported_block_info(
	...
	let babe_epoch = {
		...
		//
		// It's worth nothing that diamond session changes, at least for the purposes of parachains,
...
pub(crate) async fn handle_new_head(
	...
	for (block_hash, block_header, imported_block_info) in imported_blocks_and_info {
		...
		// cf. https://github.com/paritytech/diamond/issues/2411
...
pub(crate) mod tests {
	...
	use diamond_node_primitives::approval::{VRFOutput, VRFProof};
	use diamond_node_subsystem::messages::AllMessages;
	use diamond_node_subsystem_test_helpers::make_subsystem_context;
	use diamond_primitives::v1::{SessionInfo, ValidatorIndex};
	...
	impl AssignmentCriteria for MockAssignmentCriteria {
		fn compute_assignments(
			...
			_relay_vrf_story: diamond_node_primitives::approval::RelayVRFStory,
			_config: &criteria::Config,
			_leaving_cores: Vec<(
				CandidateHash,
				diamond_primitives::v1::CoreIndex,
				diamond_primitives::v1::GroupIndex,
			)>,
		) -> HashMap<diamond_primitives::v1::CoreIndex, criteria::OurAssignment> {
			HashMap::new()
		}

		fn check_assignment_cert(
			&self,
			_claimed_core_index: diamond_primitives::v1::CoreIndex,
			_validator_index: diamond_primitives::v1::ValidatorIndex,
			_config: &criteria::Config,
			_relay_vrf_story: diamond_node_primitives::approval::RelayVRFStory,
			_assignment: &diamond_node_primitives::approval::AssignmentCert,
			_backing_group: diamond_primitives::v1::GroupIndex,
		) -> Result<diamond_node_primitives::approval::DelayTranche, criteria::InvalidAssignment> {
			Ok(0)
		}
	}
