...
use diamond_node_primitives::{BabeAllowedSlots, BabeEpoch, BabeEpochConfiguration};
...
sp_api::mock_impl_runtime_apis! {
	impl ParachainHost<Block> for MockRuntimeApi {
		...
		fn check_validation_outputs(
			&self,
			para_id: ParaId,
			_commitments: diamond_primitives::v1::CandidateCommitments,
		) -> bool {
	...
	impl BabeApi<Block> for MockRuntimeApi {
		...
		fn submit_report_equivocation_unsigned_extrinsic(
			_equivocation_proof: sp_consensus_babe::EquivocationProof<diamond_primitives::v1::Header>,
...
fn requests_check_validation_outputs() {
	...
	let para_a = 5.into();
	let para_b = 6.into();
	let commitments = diamond_primitives::v1::CandidateCommitments::default();
	let spawner = sp_core::testing::TaskExecutor::new();
