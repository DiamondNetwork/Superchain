...
pub mod messages {
	...
	pub use diamond_node_subsystem_types::messages::*;
}
...
pub type FromOverseer<M> = diamond_overseer::gen::FromOverseer<M, OverseerSignal>;

/// Specialized subsystem instance type of subsystems consuming a particular message type.
pub type SubsystemInstance<Message> =
	diamond_overseer::gen::SubsystemInstance<Message, OverseerSignal>;

/// Sender trait for the `AllMessages` wrapper.
pub trait SubsystemSender: diamond_overseer::gen::SubsystemSender<messages::AllMessages> {}

impl<T> SubsystemSender for T where T: diamond_overseer::gen::SubsystemSender<messages::AllMessages>
{}

/// Spawned subsystem.
pub type SpawnedSubsystem = diamond_overseer::gen::SpawnedSubsystem<SubsystemError>;

/// Convenience trait specialization.
pub trait SubsystemContext:
	diamond_overseer::gen::SubsystemContext<
...
where
	T: diamond_overseer::gen::SubsystemContext<
		Signal = OverseerSignal,
		AllMessages = messages::AllMessages,
		Error = SubsystemError,
	>,
{
	type Message = <Self as diamond_overseer::gen::SubsystemContext>::Message;
	type Sender = <Self as diamond_overseer::gen::SubsystemContext>::Sender;
}
