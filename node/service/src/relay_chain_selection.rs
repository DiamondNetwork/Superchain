...
use diamond_node_subsystem_util::metrics::{self, prometheus};
use diamond_overseer::{AllMessages, Handle};
use diamond_primitives::v1::{
	Block as diamondBlock, BlockNumber, Hash, Header as diamondHeader,
};
use diamond_subsystem::messages::{
...
const MAX_FINALITY_LAG: diamond_primitives::v1::BlockNumber = 50;
...
pub struct SelectRelayChain<B: sc_client_api::Backend<diamondBlock>> {
	is_relay_chain: bool,
	longest_chain: sc_consensus::LongestChain<B, diamondBlock>,
	selection: SelectRelayChainInner<B, Handle>,
}

impl<B> Clone for SelectRelayChain<B>
where
	B: sc_client_api::Backend<diamondBlock>,
...
where
	B: sc_client_api::Backend<diamondBlock> + 'static,
{
...
impl<B> SelectChain<diamondBlock> for SelectRelayChain<B>
where
	B: sc_client_api::Backend<diamondBlock> + 'static,
{
	...
	async fn best_chain(&self) -> Result<diamondHeader, ConsensusError> {
		if !self.is_relay_chain {
...
where
	B: HeaderProviderProvider<diamondBlock>,
	OH: OverseerHandleT,
{
	...
	fn block_header(&self, hash: Hash) -> Result<diamondHeader, ConsensusError> {
...
where
	B: HeaderProviderProvider<diamondBlock> + Send + Sync,
	OH: OverseerHandleT,
{
	fn clone(&self) -> Self {
