...
//! diamond service. Specialized wrapper over substrate service.
...
use {
	...
	diamond_node_core_approval_voting::Config as ApprovalVotingConfig,
	diamond_node_core_av_store::Config as AvailabilityConfig,
	diamond_node_core_av_store::Error as AvailabilityError,
	diamond_node_core_candidate_validation::Config as CandidateValidationConfig,
	diamond_node_core_chain_selection::{
		self as chain_selection_subsystem, Config as ChainSelectionConfig,
	},
	diamond_node_core_dispute_coordinator::Config as DisputeCoordinatorConfig,
	diamond_overseer::BlockInfo,
...
#[cfg(feature = "full-node")]
pub use {
	diamond_overseer::{Handle, Overseer, OverseerConnector, OverseerHandle},
	diamond_primitives::v1::ParachainHost,
...
pub use diamond_primitives::v1::{Block, BlockId, CollatorPair, Hash, Id as ParaId};
...
pub enum Error {
	...
	Overseer(#[from] diamond_overseer::SubsystemError),
	...
	#[error(transparent)]
	Jaeger(#[from] diamond_subsystem::jaeger::JaegerError),
...
// If we're using prometheus, use a registry with a prefix of `diamond`.
fn set_prometheus_registry(config: &mut Configuration) -> Result<(), Error> {
	if let Some(PrometheusConfig { registry, .. }) = config.prometheus_config.as_mut() {
		*registry = Registry::new_custom(Some("diamond".into()), None)?;
	}
	...
	ChainSelection: 'static + SelectChain<Block>,
{
	...
	let grandpa_hard_forks = if config.chain_spec.is_gold() {
		grandpa_support::gold_hard_forks()
	} else {
		Vec::new()
	};
	...
	let rpc_extensions_builder = {
		...
		move |deny_unsafe,
		      subscription_executor: diamond_rpc::SubscriptionTaskExecutor|
		      -> Result<diamond_rpc::RpcExtension, service::Error> {
			let deps = diamond_rpc::FullDeps {
				client: client.clone(),
				pool: transaction_pool.clone(),
				select_chain: select_chain.clone(),
				chain_spec: chain_spec.cloned_box(),
				deny_unsafe,
				babe: diamond_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: diamond_rpc::GrandpaDeps {
				...
				beefy: diamond_rpc::BeefyDeps {
					beefy_commitment_stream: beefy_commitment_stream.clone(),
					subscription_executor,
				},
			};

			diamond_rpc::create_full(deps).map_err(Into::into)
		}
	};
...
{
	use diamond_node_network_protocol::request_response::IncomingRequest;
	...
	let select_chain = SelectRelayChain::new(
		...
		is_relay_chain,
		handle.clone(),
		diamond_node_subsystem_util::metrics::Metrics::register(prometheus_registry.as_ref())?,
	);
	...
	// Note: GrandPa is pushed before the diamond-specific protocols. This doesn't change
	...
	{
		use diamond_network_bridge::{peer_sets_info, IsAuthority};
...

		{
			...
			task_manager.spawn_essential_handle().spawn_blocking(
				...
				Box::pin(async move {
					...
					let forward = diamond_overseer::forward_events(overseer_client, handle);
	...
	if role.is_authority() {
		...
		let babe_config = babe::BabeParams {
			...
			create_inherent_data_providers: move |parent, ()| {
				...
				async move {
					let parachain = diamond_node_core_parachains_inherent::ParachainsInherentDataProvider::create(
						&*client_clone,
						overseer_handle,
						parent,
	...
	Dispatch: NativeExecutionDispatch + 'static,
{
	...
	let light_deps = diamond_rpc::LightDeps {
	...
	let rpc_extensions = diamond_rpc::create_light(light_deps);
