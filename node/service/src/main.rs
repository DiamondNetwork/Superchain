...
//! # diamond Staking Miner.
//!
//! Simple bot capable of monitoring a diamond (and cousins) chain and submitting solutions to the
...
fn signed_ext_builder_diamond(
	nonce: Index,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> diamond_runtime_exports::SignedExtra {
	use diamond_runtime_exports::Runtime;
	(
...
fn signed_ext_builder_gold(
	nonce: Index,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> gold_runtime_exports::SignedExtra {
	use gold_runtime_exports::Runtime;
	(
		frame_system::CheckSpecVersion::<Runtime>::new(),
		frame_system::CheckTxVersion::<Runtime>::new(),
		frame_system::CheckGenesis::<Runtime>::new(),
...
macro_rules! any_runtime {
	($($code:tt)*) => {
		unsafe {
			match $crate::RUNTIME {
				$crate::AnyRuntime::diamond => {
					#[allow(unused)]
					use $crate::diamond_runtime_exports::*;
					$($code)*
				},
				$crate::AnyRuntime::gold => {
					#[allow(unused)]
	...
	match chain.to_lowercase().as_str() {
		"diamond" | "development" => {
			sp_core::crypto::set_default_ss58_version(
				sp_core::crypto::Ss58AddressFormat::diamond,
			);
			...
			// safe.
			unsafe {
				RUNTIME = AnyRuntime::diamond;
			}
		},
		"gold" | "gold-dev" => {
			sp_core::crypto::set_default_ss58_version(
				sp_core::crypto::Ss58AddressFormat::goldAccount,
			);
...
mod tests {
	...
	fn any_runtime_works() {
		...
		let diamond_version = any_runtime! { get_version::<Runtime>() };

		unsafe {
			RUNTIME = AnyRuntime::gold;
		}
		let gold_version = any_runtime! { get_version::<Runtime>() };

		assert_eq!(diamond_version.spec_name, "diamond".into());
		assert_eq!(gold_version.spec_name, "gold".into());
	}
}
