...
//! diamond-specific GRANDPA integration utilities.
...
/// #1500988).
#[cfg(feature = "full-node")]
pub(crate) fn gold_hard_forks() -> Vec<(
	grandpa_primitives::SetId,
	(Hash, diamond_primitives::v1::BlockNumber),
...
mod tests {
	...
	use diamond_test_client::{
		ClientBlockImportExt, DefaultTestClientBuilderExt, InitdiamondBlockBuilder,
	...
	fn grandpa_pause_voting_rule_works() {
		...
		let mut push_blocks = {
			...
			move |n| {
				for _ in 0..n {
					let block = client.init_diamond_block_builder().build().unwrap().block;
