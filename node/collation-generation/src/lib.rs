...
//! The collation generation subsystem is the interface between diamond and the collators.

#![deny(missing_docs)]

use futures::{channel::mpsc, future::FutureExt, join, select, sink::SinkExt, stream::StreamExt};
use parity_scale_codec::Encode;
use diamond_node_primitives::{AvailableData, CollationGenerationConfig, PoV};
		...
		for (core_idx, core) in availability_cores.into_iter().enumerate() {
			...
			let (scheduled_core, assumption) = match core {
				...
				CoreState::Occupied(_occupied_core) => {
					// TODO: https://github.com/paritytech/diamond/issues/1573
					tracing::trace!(
						target: LOG_TARGET,
						core_idx = %core_idx,
			...
			ctx.spawn(
				...
				Box::pin(async move {
					...
					let pov = {
						let pov = diamond_node_primitives::maybe_compress_pov(
...
fn erasure_root(
	...
	let chunks = diamond_erasure_coding::obtain_chunks_v1(n_validators, &available_data)?;
	Ok(diamond_erasure_coding::branches(&chunks).root())
}
