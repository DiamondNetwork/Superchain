...
The testing strategy for diamond is 4-fold:
...
prometheus avoiding additional diamond source changes.
...
For full examples on how to use [`grcov` /w diamond specifics see the github repo](https://github.com/mozilla/grcov#coverallscodecov-output).
...
fn main() -> eyre::Result<()> {
	...
	diamond_cli::run_node(cli, BehaveMaleficient)?;
	Ok(())
}
```
