...
diamond_path = ENV['GITHUB_WORKSPACE'] + '/diamond/'

# Generate an ERB renderer based on the template .erb file
renderer = ERB.new(
  File.read(ENV['GITHUB_WORKSPACE'] + '/diamond/scripts/github/diamond_release.erb'),
  trim_mode: '<>'
)

# get ref of last diamond release
...
all_changes = (diamond_cl.changes + substrate_cl.changes).reject do |c|
  c[:title] =~ /[Cc]ompanion/
end
...
diamond_json = JSON.parse(
  File.read(
    "#{ENV['GITHUB_WORKSPACE']}/diamond-srtool-json/diamond_srtool_output.json"
  )
)

gold_json = JSON.parse(
  File.read(
    "#{ENV['GITHUB_WORKSPACE']}/gold-srtool-json/gold_srtool_output.json"
  )
)
