...
#   echo "diamond-js-api required; try"
#   echo "  sudo yarn global add @diamond/api-cli"
#   exit 1
# fi
...
diamond="target/release/diamond"
adder_collator="target/release/adder-collator"

# ensure the diamond binary exists and is up to date
if [ ! -x "$diamond" ] || [ "$diamond" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the diamond binary, in un-flagged form,
# one of:
...
# doesn't work yet due to https://github.com/diamond-js/tools/issues/185
# diamond-js-api \
