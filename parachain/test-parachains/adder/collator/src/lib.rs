...
use diamond_primitives::v1::{CollatorId, CollatorPair};
...
impl Collator {
	...
	pub fn create_collation_function(
		...
		Box::new(move |relay_parent, validation_data| {
			...
			let compressed_pov = diamond_node_primitives::maybe_compress_pov(pov);
...
mod tests {
	...
	use diamond_parachain::primitives::{ValidationParams, ValidationResult};
	use diamond_primitives::v1::PersistedValidationData;
	...
	fn validate_collation(collator: &Collator, parent_head: HeadData, collation: Collation) {
		use diamond_node_core_pvf::testing::validate_candidate;
